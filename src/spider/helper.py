    ...
    def update_domain_url_status(robot_parser, domain):
        try:
            with constants.DOMAIN_MODEL_LOCK:
                ...
                (
                    DomainModel.update({DomainModel.url_status_id: url_status})
    ...
    def add_crawl_email_database():
        with constants.CRAWL_EMAILS_LOCK:
            while len(Helper.crawl_emails) > 0:
                ...
                try:
                    mass_insert_query = (
                        CrawlEmailModel.insert_many(email_objects)
                        .on_conflict(action="IGNORE")
                        .as_rowcount()
                        .execute()
                    )
                    logging.info(
                        f"Inserted {mass_insert_query} emails to crawl_email. {len(Helper.crawl_emails)=}"
                    )
                except Exception as e:
